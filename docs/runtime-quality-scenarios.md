...
## Beauty Debug Overlay の確認手順

1. サンプルプリセットで `ナチュラル美顔` を選択する
2. `Beauty Debug Overlay` を `美肌範囲` に変更し、顔領域に半透明マスクが重なることを確認する
3. `変形範囲` に変更し、active operations の影響範囲（円/線/領域）が表示されることを確認する
4. `角度弱化` に変更し、横顔で黄色オーバーレイが濃くなることを確認する
5. `安定性` に変更し、顔を外した時に赤オーバーレイが濃くなることを確認する
